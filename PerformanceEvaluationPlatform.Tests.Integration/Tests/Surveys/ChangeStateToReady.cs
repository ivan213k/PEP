...
{
    [TestFixture]
    public class ChangeStateToReady : IntegrationTestBase
    {
        [Test]
        public async Task Request_should_return_not_found_when_wrong_id_is_requested()
        {
            //Arrange
            var surveyId = 10000;
            var request = CreatePutHttpRequest("surveys", surveyId, "ready");
        ...
        [Test]
        public async Task Request_should_return_unprocessable_entity_when_survey_not_in_draft_state() 
        {
            //Arrange
            var surveyId = 2;
            var request = CreatePutHttpRequest("surveys", surveyId, "ready");
        ...
        [Test]
        public async Task Request_should_change_state_to_ready() 
        {
            //Arrange
            var surveyId = 1;
            var request = CreatePutHttpRequest("surveys", surveyId, "ready");
            ...
            Assert.AreEqual("Ready", survey.State);
        }
