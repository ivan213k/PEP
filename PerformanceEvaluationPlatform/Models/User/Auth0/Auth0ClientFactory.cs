...
namespace PerformanceEvaluationPlatform.Models.User.Auth0
{
    public class Auth0ClientFactory : IAuth0ClientFactory
    {
        ...
        public Auth0ClientFactory(IOptions<Auth0Configure> config,IMemoryCache cache)
        {
        ...
        public async Task<ManagementApiClient> Create()
        {
            ...
            if (!_cache.TryGetValue(cacheKey,out token))
            {
                ...
                _cache.Set(cacheKey, token, new MemoryCacheEntryOptions().SetAbsoluteExpiration(TimeSpan.FromHours(1)));
            }
